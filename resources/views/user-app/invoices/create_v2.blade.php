...
@section('content')
    <div class="max-w-7xl mx-auto" x-data="invoiceForm()" x-init="init()">
        ...
        <div class="mb-6">
            <div class="flex items-start justify-between gap-4">
                ...
                <div class="hidden md:flex items-center gap-2">
                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-primary-50 text-primary-700 border border-primary-100">Draft</span>
                </div>
            </div>

            <div class="mt-4 grid grid-cols-2 md:grid-cols-4 gap-2">
                <div class="flex items-center gap-2 rounded-lg border border-primary-200 bg-primary-50 px-3 py-2">
                    <span class="flex h-6 w-6 items-center justify-center rounded-full bg-primary-600 text-white text-xs">1</span>
                    <span class="text-xs font-medium text-primary-700">Invoice details</span>
                </div>
                <div class="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2">
                    <span class="flex h-6 w-6 items-center justify-center rounded-full bg-gray-200 text-gray-700 text-xs">2</span>
                    <span class="text-xs font-medium text-gray-700">Customer</span>
                </div>
                <div class="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2">
                    <span class="flex h-6 w-6 items-center justify-center rounded-full bg-gray-200 text-gray-700 text-xs">3</span>
                    <span class="text-xs font-medium text-gray-700">Items</span>
                </div>
                <div class="hidden md:flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2">
                    <span class="flex h-6 w-6 items-center justify-center rounded-full bg-gray-200 text-gray-700 text-xs">4</span>
                    <span class="text-xs font-medium text-gray-700">Review</span>
                </div>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('user.invoices.store') }}" @submit="syncItems()" class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            ...
            <!-- Body: optimized layout for wider Items -->
            <div class="p-6">
                <div class="grid grid-cols-1 lg:grid-cols-[360px_minmax(0,1fr)] gap-6 items-start">
                    <!-- Left column (meta + customer) -->
                    <div class="space-y-8">
                        <!-- From (Company) -->
                    ...
                    <div>
                        ...
                        <div class="space-y-2 mt-6">
                            ...
                            <textarea name="notes" rows="4" class="block w-full !px-4 !py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500" placeholder="Additional information (optional)">{{ old('notes') }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            <!-- Sticky action bar (totals + actions) -->
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div class="flex items-center gap-6">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-xl font-extrabold text-gray-900" x-text="formatMoney(grandTotal())"></span>
                        </div>
                    </div>
